...
function networkUp() {
  ...
  if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
    ...
    export BYFN_CA1_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/gast1.gast-network.com/ca && ls *_sk)
    export BYFN_CA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/gast2.gast-network.com/ca && ls *_sk)
  fi
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.gast-network.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.gast-network.com
    docker cp -a orderer.gast-network.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.gast-network.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.gast-network.com

    for PEER in peer0.gast1.gast-network.com peer1.gast1.gast-network.com peer0.gast2.gast-network.com peer1.gast2.gast-network.com; do
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/gast1.gast-network.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/gast2.gast-network.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# (``peer0.gast1.gast-network.com`` & ``peer0.gast2.gast-network.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for Gast1MSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/Gast1MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Gast1MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for Gast1MSP..."
    exit 1
  fi
  ...
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for Gast2MSP..."
    exit 1
  fi
  echo
}
